...
def read_observed_errors(csv_path):
    """读取 Data block 的 stat+ 和 stat- 列，返回 (stat_up, stat_down) 均为正值。"""
    stat_up = []
    stat_down = []
    current_block = None
    ...
    with open(csv_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if len(parts) > 3 and parts[3].strip() == 'Data [counts per bin]':
                current_block = 'data'
                continue
            if current_block == 'data' and len(parts) > 3 and parts[3].strip().startswith('Background'):
                break

            if current_block == 'data':
                try:
                    stat_up.append(float(parts[4]))
                    stat_down.append(abs(float(parts[5])))
                except (ValueError, IndexError):
                    continue

    return np.array(stat_up), np.array(stat_down)
...
if __name__ == "__main__":
    ...
    test_file = os.path.join(DATA_DIR, 'HEPData-ins3088922-v1-Unconstrained_14_channels.csv')
    ...
    # 读取绝对协方差矩阵（即原始协方差矩阵）
    test_filecov = os.path.join(DATA_DIR, 'HEPData-ins3088922-v1-14_channel_covariance_matrix.csv')
