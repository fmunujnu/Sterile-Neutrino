...
- NuMI, BNB pi0/NC channels, and multi-experiment combinations remain diagnostic until their own declared inputs and validations exist. The parallel `one_plus_three_plus_one/` package is a development analysis which reuses those declared inputs; it must not be described as collaboration-exact or validated merely because it runs.
...
## Repository and run classification

- `src/sterile_fit/` contains importable core code only. It must never contain
  paper-specific plot assembly, downloaded-PDF extraction, completed-result
  comparison, runtime benchmarks or historical scripts.
- `scripts/scan.py` is the single 3+1 scan engine. `scripts/runs/run1_non_toy.py`
  and `scripts/runs/run2_toy_mc.py` are the only primary user-facing inference
  entries; they may set calibration and output defaults but must not duplicate
  prediction, covariance, profile, CLs or plotting logic.
- The 1+3+1 development path is intentionally parallel and isolated:
  `src/sterile_fit/one_plus_three_plus_one/` and
  `scripts/one_plus_three_plus_one/`. It may call the established experiment,
  covariance and CLs APIs, but must not modify, hide behind legacy names, or
  silently generalize the existing 3+1 parameter/model/profile code.
- Run 1 means the analytic moment-Gaussian CLs approximation with no
  pseudo-experiments. Run 2 means empirical Toy-MC CLs under both hypotheses.
  Never place adaptive/hybrid values in Run 2 without marking unevaluated points
  as analytic rather than Toy.
- Reusable experiment preparation and spectrum commands stay under
  `scripts/experiments/<detector>/<beam>/`. Completed-result overlays, official
  grid comparisons, PDF path extraction, benchmarks and other one-off work live
  under `studies/<purpose>/` and cannot be imported by core code.
- Generated results are classified as `outputs/run1_non_toy/`,
  `outputs/run2_toy_mc/`, `outputs/spectra/`, `outputs/checks/`, or
  `outputs/archive/legacy_runs/`. Do not create new `production/`,
  `paper_reproduction/`, `final_vN/` or timestamp trees outside this contract.
...
outputs/run1_non_toy/
    Regenerable analytic non-Toy prefit/profile/parameter-space results.

outputs/run2_toy_mc/
    Regenerable empirical Toy-MC profile/parameter-space results.
...
public/shared release + beam inputs
    ...
    -> outputs/spectra, outputs/run1_non_toy or outputs/run2_toy_mc
```
...
- Parallelize production Toy scans at the independent scan-point level with
  `--scan-workers`, prefer the `processes` backend for CPU-heavy profiles, and
  keep `--toy-workers 1`.  Never nest point-level and within-point parallelism.
  Progress reporting must expose completed points, elapsed wall time, estimated
  total wall time and remaining wall time without changing seeded calculations.
