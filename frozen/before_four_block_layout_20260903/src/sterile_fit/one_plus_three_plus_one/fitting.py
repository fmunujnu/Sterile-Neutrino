...
def prefit_one_plus_three_plus_one(
    objective: Objective,
    *,
    seed: int = 42,
    maxiter: int = 600,
    ...
    """Global seven-coordinate prefit including both 3+1 limits and the null."""
    controls = {"maxiter": maxiter, "popsize": popsize}
    candidates = [
        profile_one_plus_three_plus_one(objective, {}, seed=seed, **controls).best_fit,
        profile_one_plus_three_plus_one(
            objective,
            {"abs_Ue4_squared": 0.0, "abs_Umu4_squared": 0.0, "cp_phase_mue_rad": 0.0},
            seed=seed + 10_000,
            **controls,
        ).best_fit,
        profile_one_plus_three_plus_one(
            objective,
            {"abs_Ue5_squared": 0.0, "abs_Umu5_squared": 0.0, "cp_phase_mue_rad": 0.0},
            seed=seed + 20_000,
            **controls,
        ).best_fit,
    ]
    null = OnePlusThreePlusOneParameters.three_neutrino_null()
    candidates.append(OnePlusThreePlusOneFitPoint(null, float(objective(null))))
