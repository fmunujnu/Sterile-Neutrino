...
Run 1：不生成 Toy，使用解析矩匹配高斯近似快速计算 CLs：

```powershell
python scripts/runs/run1_non_toy.py --mode appearance-profile
```

Run 2：在 3ν 与被检验 4ν 假设下生成 Toy，并在每份 Toy 内重复 profile：

```powershell
python scripts/runs/run2_toy_mc.py --mode appearance-profile --number-of-toys 100
```

两个入口只锁定统计校准和默认输出位置；预测、协方差、profile、CLs 与绘图仍只由
`scripts/scan.py` 这一套核心引擎实现。Run 1 默认写入 `outputs/run1_non_toy/`，
Run 2 默认写入 `outputs/run2_toy_mc/`。完整分类见
...
Run 1 扫描结果写入 `outputs/run1_non_toy/<运行目录>/result.csv`，Run 2 写入
`outputs/run2_toy_mc/<运行目录>/result.csv`；运行说明均为同目录的 `metadata.json`。
...
```powershell
python scripts/runs/generate_joint_figures.py --grid-points 61
```
...
```powershell
python scripts/runs/run2_toy_mc.py `
  --mode appearance-profile `
  --analysis-config configs/analyses/microboone_bnb_numi.yaml `
  --number-of-toys 100 `
  --toy-seed 20250821 `
  --scan-workers 24 `
  --scan-parallel-backend processes `
  --toy-workers 1 `
  --toy-batch-size 256
```
...
`--store-toy-distributions`，程序会为每个扫描点写一个可见 CSV。优先用
`--scan-workers` 并行彼此独立的扫描点，并保持 `--toy-workers 1`。CPU 密集的
profile 推荐 `--scan-parallel-backend processes`，绕开 Python 线程锁；扫描点种子由
`--toy-seed` 和固定点序号确定，因此改变线程数不会改变随机
...
并行运行时，每完成一个扫描点都会立即打印完成比例、当前总耗时、根据已完成点平均
速度估计的总耗时和剩余时间。开头少量点的估计会波动，完成点增多后才逐渐稳定。
程序禁止同时令 `--scan-workers` 和 `--toy-workers` 大于 1，防止嵌套线程耗尽 CPU
和内存而反而减速。
...
scripts/runs/run1_non_toy.py                          non-Toy 解析 CLs 正式入口
scripts/runs/run2_toy_mc.py                           Toy-MC CLs 正式入口
...
scripts/scan.py                                       两个 run 共用的唯一扫描引擎
```
...
outputs/run1_non_toy/                non-Toy解析CLs结果
outputs/run2_toy_mc/                 Toy-MC CLs结果
...
1+3+1 的第一阶段实现位于 `src/sterile_fit/one_plus_three_plus_one/`，运行入口位于
`scripts/one_plus_three_plus_one/`。它只替换概率模型，并已用回归测试要求第二惰性态
混合为零时逐点回到 3+1；数据、kernel、背景、Reco、协方差与原 3+1 代码均不复制、
不改变。用法与当前物理边界见 `scripts/one_plus_three_plus_one/README.md`。
