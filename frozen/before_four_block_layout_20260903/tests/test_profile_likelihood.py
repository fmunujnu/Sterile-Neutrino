...
def test_fixed_electron_disappearance_profiles_both_s14_branches() -> None:
    target = 0.36

    def prefers_large_branch(parameters: ThreePlusOneParameters) -> float:
        return (parameters.sin2_theta14 - 0.9) ** 2 + (parameters.sin2_theta24 - 0.3) ** 2

    result = profile_s14_s24_at_fixed_sin2_2theta_ee(
        prefers_large_branch,
        delta_m2_41_eV2=1.0,
        sin2_2theta_ee=target,
    )
    assert result.best_fit.parameters.sin2_2theta_ee_exact == pytest.approx(target)
    assert result.best_fit.parameters.sin2_theta14 == pytest.approx(0.9)
    assert result.best_fit.parameters.sin2_theta24 == pytest.approx(0.3, abs=1e-5)


def test_electron_disappearance_grid_retains_both_scan_coordinates() -> None:
    results = profile_electron_disappearance_grid(_objective, [0.8, 1.2], [0.1])
    assert [item.delta_m2_41_eV2 for item in results] == [0.8, 1.2]
    assert all(item.sin2_2theta_ee == 0.1 for item in results)
