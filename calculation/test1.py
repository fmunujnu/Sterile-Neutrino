...
class LikelihoodCalculator:
    ...
    def log_likelihood(self, x):
        ...
        n = 130  # 要添加的0的个数
        zeros = np.zeros(n, dtype=S3.dtype)  # 保持数据类型一致
        S4 = np.append(S3, zeros)

        bkg1 = bkg[:182]

        cov_f1 = cov_f[:182, :182]

        n_obs1 = n_obs[:182]

        mu    = S4 + bkg1
        mu    = np.maximum(mu, 1e-12)
        cov_a = cov_f1 * np.outer(mu, mu)
        n     = cov_a.shape[0]
