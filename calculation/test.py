"""
MicroBooNE / MiniBooNE 1+3+1 立体中微子振荡分析
=================================================
最一般化的 signal_scales 与 channel 配置范例
=============================================
...
─────────────────────────────────────────────────────────────
  channel 编号  │  类型              │  预测构成
─────────────────────────────────────────────────────────────
  0             │  signal (scale=1.0)│  bkg + 1.0 × S[26]
  1             │  signal (scale=0.6)│  bkg + 0.6 × S[26]
  2             │  signal (scale=0.3)│  bkg + 0.3 × S[26]
  3             │  background-only   │  bkg（约束本底形状）
  4             │  background-only   │  bkg（约束本底形状）
  5             │  background-only   │  bkg（约束本底形状）
  6             │  background-only   │  bkg（约束本底形状）
  7             │  background-only   │  bkg（约束本底形状）
  8~13          │  inactive          │  不参与 χ² 计算
─────────────────────────────────────────────────────────────

【比例因子物理含义】
  scale = 1.0 → channel 0：νe CC0π 主信号道，完整振荡信号
  scale = 0.6 → channel 1：部分信号纯度（混入本底事件）
  scale = 0.3 → channel 2：低纯度道（如 π⁰ 混合道）
  scale = 0.0 → 等效于将该 channel 移出 signal_channels（退化为纯本底）
  scale > 1.0 → 多重计数或信号增强道（慎用）

【χ² 计算范围】
  active_channels = [0,1,2,3,4,5,6,7]  → 8 ch × 26 bins = 208 维
  inactive channels [8..13] 完全不影响 χ² 结果
"""
...
N_CHANNELS       = 14
N_BINS_PER_CHAN  = 26
N_TOTAL          = N_CHANNELS * N_BINS_PER_CHAN   # 364
...
# 辅助函数
# ============================================================
def channel_slice(ch: int) -> slice:
    start = ch * N_BINS_PER_CHAN
    return slice(start, start + N_BINS_PER_CHAN)


def channels_to_indices(channel_list: list) -> np.ndarray:
    indices = []
    for ch in channel_list:
        start = ch * N_BINS_PER_CHAN
        indices.extend(range(start, start + N_BINS_PER_CHAN))
...
class ExperimentBase:
    ...
    def get_observations_full(self): return self.observed.copy()
    def get_background_full(self):   return self.background.copy()
    def get_covariance_full(self):   return self.covariance.copy()
...
########################################################################################################################
    ...
    def _post_init_obs(self):
        self.obs_Elo_GeV     = self.obs_Elo
        self.obs_Ehi_GeV     = self.obs_Ehi
...
def oscillation_probability_array(U, m2, alpha, beta, L, E_arr, anti=False):
    ...
    coeffs = (np.conj(U[beta, :]) * U[alpha, :] if anti
              else U[beta, :] * np.conj(U[alpha, :]))
...
class EventRateCalculator:
    def __init__(self, exp: ExperimentBase, model_wrapper: OscillationModelWrapper):
        self.exp      = exp
    ...
    def compute_true_spectrum(self, U, m2) -> np.ndarray:
        P_nu  = oscillation_probability_array(U, m2, 1, 0, self.L_km, self.E_true, anti=False)
        P_nub = oscillation_probability_array(U, m2, 1, 0, self.L_km, self.E_true, anti=True)
        return self.pc_nu * P_nu + self.pc_nub * P_nub

    def compute_signal_26bins(self, model_name: str, params: dict) -> np.ndarray:
        U, m2, _ = self.model.get_mixing_and_masses(model_name, params)
        true_spectrum = self.compute_true_spectrum(U, m2)
        return self.R.T @ true_spectrum
...
class LikelihoodCalculator:
    ...
    def __init__(
        ...
        assert all(0 <= ch < N_CHANNELS for ch in signal_channels)
        assert all(0 <= ch < N_CHANNELS for ch in active_channels)
        assert len(active_channels) > 0

        self.signal_channels     = list(signal_channels)
        self.active_channels     = list(active_channels)
        self.background_channels = [ch for ch in active_channels if ch not in signal_channels]

        # ── signal_scales：为每个 signal channel 独立设定比例因子 ──
        # 默认全部为 1.0；传入的 signal_scales 字典覆盖对应 channel 的值
        self.signal_scales = {ch: 1.0 for ch in signal_channels}
        if signal_scales is not None:
            self.signal_scales.update(signal_scales)

        self.obs_full = exp.get_observations_full()
        self.bkg_full = exp.get_background_full()
        self.cov_full = exp.get_covariance_full()

        self._active_indices = channels_to_indices(self.active_channels)
        self._obs_active     = self.obs_full[self._active_indices]
        self._bkg_active     = self.bkg_full[self._active_indices]
        self._cov_active     = self.cov_full[np.ix_(self._active_indices, self._active_indices)]
    ...
    # ── 核心：构建 364 维预测向量 ──────────────────────────────
    def _build_mu_full(self, params: dict) -> np.ndarray:
        """
        构建 364 维预测事件率向量 μ。

        μ[ch] = bkg[ch] + signal_scales[ch] × S[26]   （ch ∈ signal_channels）
        μ[ch] = bkg[ch]                                （ch ∉ signal_channels）

        signal_scales[ch] ∈ [0, ∞)，通常物理范围为 [0.0, 1.0]：
          0.0 → 该 channel 无信号（等效于纯本底）
          1.0 → 完整振荡信号
          0~1 → 部分纯度 / 效率折扣
        """
        signal_26 = self.rate_calc.compute_signal_26bins(self.model_name, params)
        mu_full   = self.bkg_full.copy()

        for ch in self.signal_channels:
            scale     = self.signal_scales.get(ch, 1.0)   # 取对应 channel 的比例因子
            sl        = channel_slice(ch)
            mu_full[sl] += scale * signal_26
            mu_full[sl]  = np.maximum(mu_full[sl], 1e-12)
    ...
    def log_likelihood(self, x) -> float:
        params     = self._v2p(x)
        mu_full    = self._build_mu_full(params)
        mu_active  = mu_full[self._active_indices]
        obs_active = self._obs_active
        cov_reg    = self._cov_active.copy()
        eps        = 1e-8 * np.trace(cov_reg) / self._n_active
        cov_reg   += eps * np.eye(self._n_active)
        try:
            inv_c = np.linalg.inv(cov_reg)
        except np.linalg.LinAlgError:
            inv_c = np.linalg.pinv(cov_reg)
        d = obs_active - mu_active
        return 0.5 * float(d @ inv_c @ d)

    # ── 零假设 χ² ──────────────────────────────────────────────
    def compute_chi2_null(self) -> float:
        bkg_active = self._bkg_active
        obs_active = self._obs_active
        cov_reg    = self._cov_active.copy()
        eps        = 1e-8 * np.trace(cov_reg) / self._n_active
        cov_reg   += eps * np.eye(self._n_active)
    ...
    def global_minimize(self, n_random=50, seed=42):
        ...
        if best_x is None:
            best_x  = x0
            best_c2 = 2. * self.log_likelihood(x0)
    ...
    # ── Profile likelihood ─────────────────────────────────────
    def profile_likelihood(self, fixed_params: dict, x0_free=None,
                            n_random_starts=3, seed=None):
        ...
        starts = [np.array([self._to_opt(self.param_info[nm]['init'], nm) for nm in fr_nms])]
        if x0_free is not None:
            starts.append(np.array(x0_free))
        if self._global_best_params is not None:
            gx = np.array([self._to_opt(
                self._global_best_params.get(nm, self.param_info[nm]['init']), nm)
                for nm in fr_nms])
    ...
    # ── 2D 网格扫描 ────────────────────────────────────────────
    def scan_physical_2d(self, grid1, grid2, mode=1, fixed_base=None,
        ...
        for i, v1 in enumerate(grid1):
            for j, v2 in enumerate(grid2):
                p = dict(tmpl)
                scan_keys = self._get_mode_scan_keys(mode)
                for k_name, val in fixed_base.items():
                    if k_name in p and k_name not in scan_keys:
                        p[k_name] = val
    ...
    def scan_physical_2d_profile(self, grid1, grid2, mode=1, fixed_base=None,
                                  chi2_null_override=None):
        ...
        for i, v1 in enumerate(grid1):
            for j, v2 in enumerate(grid2):
                fp              = self._build_fixed_params_for_profile(mode, v1, v2, fixed_base)
                _, neg_logL     = self.profile_likelihood(fp)
                chi2_grid[i, j] = -2. * neg_logL
                param_store[i][j] = dict(fp)
    ...
    @staticmethod
    def _get_mode_scan_keys(mode) -> set:
    ...
    @staticmethod
    def params_to_plot_coords(params: dict, mode: int):
...
def _compute_ts_at_projection(g1, g2, chi2_grid, chi2_ref, proj_xy,
    ...
    di = (int(np.argmin(np.abs(np.log10(g1) - np.log10(px)))) if xscale == 'log'
          else int(np.argmin(np.abs(g1 - px))))
    dj = (int(np.argmin(np.abs(np.log10(g2) - np.log10(py)))) if yscale == 'log'
          else int(np.argmin(np.abs(g2 - py))))
    c2 = chi2_grid[di, dj]
...
# 绘图函数
# ============================================================
def _mode_axis_info(mode: int) -> dict:
    info = {
...
def plot_2d_contour(grid1, grid2, TS_matrix, mode=1,
                    ...
                    signal_channels=None, active_channels=None,
                    signal_scales=None,
                    savepath=None):
    ...
    crit68, crit95, crit99 = [chi2_dist.ppf(cl, df=2) for cl in [0.68, 0.95, 0.99]]
    chi2_crits = [crit68, crit95, crit99]
    cl_labels = ['68% CL', '95% CL', '99% CL']
    cl_ls = [':', '-', '--']
    ts_max = float(np.nanmax(TS_matrix))

    sq = 5.5; cb_gap = 0.10; cb_w = 0.22
    pad_l = 0.85; pad_r = 0.35; pad_t = 0.75; pad_b = 0.80; gap = 1.5
    ...
    ax1.set_title(title + r'  $\Delta\chi^2$ map', fontsize=10, pad=8)
    ...
    ax2.set_title(title + '  Allowed regions', fontsize=10, pad=8)
...
# ============================================================
# ██████████████████████████████████████████████████████████
#  最一般化的 channel 配置与比例因子设定范例
# ██████████████████████████████████████████████████████████
# ============================================================
#
# 【三类 channel 总览（14 个 channel，编号 0~13）】
#
#  ┌─────────────────────────────────────────────────────────┐
#  │  signal_channels  + signal_scales                       │
#  │    channel 0  → scale = 1.0  (νe CC0π 主信号道)        │
#  │    channel 1  → scale = 0.6  (部分信号纯度道)           │
#  │    channel 2  → scale = 0.3  (低纯度混合道)             │
#  ├─────────────────────────────────────────────────────────┤
#  │  background-only channels（active 但不在 signal 中）    │
#  │    channel 3, 4, 5, 6, 7  → 纯本底，约束形状            │
#  ├─────────────────────────────────────────────────────────┤
#  │  inactive channels（完全不参与 χ² 计算）                │
#  │    channel 8~13  → 不出现在 active_channels 中          │
#  └─────────────────────────────────────────────────────────┘
#
# 【χ² 计算维度】
#   active_channels = [0,1,2,3,4,5,6,7]
#   参与计算的 bin 总数 = 8 × 26 = 208
#
# 【修改方法】
#   仅需更改下方三个变量，其余代码无需改动：
#     SIGNAL_CHANNELS  → 哪些 channel 叠加振荡信号
#     ACTIVE_CHANNELS  → 哪些 channel 参与 χ² 计算
#     SIGNAL_SCALES    → 每个 signal channel 叠加多少比例的信号
# ============================================================
####################################################################################
# ── 比例因子设定（最一般化：每个 signal channel 独立指定）──────
SIGNAL_CHANNELS = [0,1]          # 叠加振荡信号的 channel 列表
ACTIVE_CHANNELS = [0,1,2,3,4,5,6]     # 参与 χ² 计算的 channel 列表（0~6）

SIGNAL_SCALES = {
    # channel 编号 : 振荡信号叠加比例因子
    # ─────────────────────────────────────────────────────
    # 物理含义：μ[ch] = bkg[ch] + scale × S_osc[26]
    # ─────────────────────────────────────────────────────
    0: 0.7,   # νe CC0π 主信号道：完整振荡信号（100%）
    1: 0.3,   # 部分纯度道：叠加 60% 的振荡信号（效率折扣 / 信号污染修正）
    # ─────────────────────────────────────────────────────
    # 可选示例（取消注释以使用）：
    # 3: 0.0,   # scale=0.0 → 等效于纯本底（即使在 signal_channels 中也无信号贡献）
    # 4: 1.5,   # scale>1.0 → 信号增强（如多振荡来源叠加，慎用）
    # ─────────────────────────────────────────────────────
}
...
def print_channel_summary(signal_channels, active_channels, signal_scales):
    """打印 channel 配置的详细摘要，便于核查。"""
    bkg_only = [ch for ch in active_channels if ch not in signal_channels]
    inactive = [ch for ch in range(N_CHANNELS) if ch not in active_channels]

    print("\n" + "=" * 65)
    print("  Channel 配置摘要")
    print("=" * 65)
    print(f"  {'channel':>9}  {'类型':^18}  {'比例因子':>10}  {'预测构成'}")
    print("  " + "-" * 61)
    for ch in range(N_CHANNELS):
        if ch in signal_channels:
            scale = signal_scales.get(ch, 1.0)
            tag   = "signal"
            pred  = f"bkg + {scale:.2g} × S_osc"
            active_mark = "✓ active"
        elif ch in active_channels:
            scale = 0.0
            tag   = "background-only"
            pred  = "bkg（纯本底）"
            active_mark = "✓ active"
            ...
            tag   = "inactive"
            pred  = "—（不参与 χ²）"
            active_mark = "✗ inactive"
        print(f"  ch {ch:>2}  │  {tag:^16}  │  {scale:>8.2g}  │  {pred}   [{active_mark}]")
    print("  " + "-" * 61)
    print(f"  active bins 总数  : {len(active_channels)} ch × {N_BINS_PER_CHAN} bins = "
          f"{len(active_channels) * N_BINS_PER_CHAN} 维")
    print(f"  signal channels   : {signal_channels}")
    print(f"  background-only   : {bkg_only}")
...
def main():
    ...
    # ── 打印 channel 配置摘要 ─────────────────────────────────
    print_channel_summary(SIGNAL_CHANNELS, ACTIVE_CHANNELS, SIGNAL_SCALES)

    # ── 构建似然计算器 ────────────────────────────────────────
    # ┌─────────────────────────────────────────────────────┐
    # │  signal_channels  : 振荡信号叠加到哪些 channel      │
    # │  active_channels  : 哪些 channel 参与 χ² 计算       │
    # │  signal_scales    : 每个 signal channel 的比例因子  │
    # │                     （缺省值 1.0，下面给出完整设定） │
    # └─────────────────────────────────────────────────────┘
    likelihood = LikelihoodCalculator(
        ...
        model_name      = "1+3+1",
        param_settings  = main_param_settings,
        signal_channels = SIGNAL_CHANNELS,   # [0, 1, 2]
        active_channels = ACTIVE_CHANNELS,   # [0..7]
        signal_scales   = SIGNAL_SCALES,     # {0:1.0, 1:0.6, 2:0.3}
    )

    # ── 确认内部比例因子设定（调试用） ───────────────────────
    print("\n  内部 signal_scales 确认：")
    for ch in SIGNAL_CHANNELS:
        print(f"    channel {ch}  scale = {likelihood.signal_scales[ch]:.4g}")
    ...
    print("\n" + "=" * 65)
    print("  Best-Fit Results  (1+3+1 model)")
    print("=" * 65)
    ...
    for m in modes_to_run:
        ...
        print(f"\n  Mode {m} 扫描中... "
              f"({len(g1)}×{len(g2)} = {len(g1)*len(g2)} 格点)")
    ...
    for m in modes_to_run:
        ...
        plot_2d_contour(
            ...
            signal_scales=SIGNAL_SCALES,       # 传入绘图函数以显示在标题中
