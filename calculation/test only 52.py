...
def plot_bkg_pred_vs_data(exp, rate_calc, model_name, params,
    ...
    # ── 各分量汇总 ────────────────────────────────────────────────
    # 仅保留前两通道（FC=通道0, PC=通道1），所有通道均有信号贡献
    ...
    ax.step(
        ...
        label='Background (FC + PC channels sum)'
    )
    ...
    # --- 观测数据阶梯线（红色实线，与预测线样式相同） ---
    D_step  = np.append(data_sum, data_sum[-1])
    ax.step(E_step, D_step, where='post',
...
PARAM_SETTINGS = {
    'dm41'   : {'init': 0.87,  'lower': 0.01,  'upper': 10.,         'nsteps': 100, 'scale': 'log'},
    'dm51'   : {'init': 0.47,  'lower': 0.01,  'upper': 10.,         'nsteps': 50,  'scale': 'log'},
