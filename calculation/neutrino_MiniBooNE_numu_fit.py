...
class MiniBooNE:
    def __init__(self):
        ...
        obs_Elo = np.array([475., 550., 675., 800., 950., 1100., 1300., 1500.])
        obs_Ehi = np.array([550., 675., 800., 950., 1100., 1300., 1500., 3000.])
        self.obs_Elo_GeV     = obs_Elo / 1000.
        self.obs_Ehi_GeV     = obs_Ehi / 1000.
        ...
        self.dE_true = np.diff(np.concatenate([[self.Elo[0]], self.Ehi]))
...
class EventRateCalculator:
    ...
    def compute_signal_spectrum(self, model_name, params):
        ...
        return self.R.T @ (self.pc_nu * P_nu + self.pc_nub * P_nub)
...
def plot_bkg_pred_vs_data(exp, rate_calc, model_name, params,
    ...
    Parameters
    ----------
    exp         : MiniBooNE 实验对象，提供 background / observed / obs_centers_GeV
                  以及 obs_Elo_GeV / obs_Ehi_GeV（用于误差棒宽度）。
    rate_calc   : EventRateCalculator 对象，用于计算指定参数点的信号谱。
    model_name  : str，模型名称（如 "1+3+1"）。
    params      : dict，模型参数字典（传入指定参数点）。
    param_label : str or None，显示在图例 / 标题中的参数描述文字；
                  若为 None，则自动从 params 中提取关键参数生成。
    savepath    : str or None，若不为 None，则将图保存到该路径。
    ...
    """
    # ── 计算各分量 ────────────────────────────────────────────────
    bkg       = exp.background                                        # 纯本底（8 bins）
    signal    = rate_calc.compute_signal_spectrum(model_name, params) # 振荡信号（8 bins）
    total_pred = bkg + signal                                         # 本底 + 预测信号
    data      = exp.observed                                          # 观测数据（8 bins）

    energies  = exp.obs_centers_GeV                                   # 能量 bin 中心（GeV）
    xerr_lo   = energies - exp.obs_Elo_GeV                            # 左侧半宽
    xerr_hi   = exp.obs_Ehi_GeV - energies                            # 右侧半宽

    # ── 统计误差（泊松近似：sqrt(N)） ────────────────────────────
    data_err      = np.sqrt(np.maximum(data, 1.))
    ...
    ax.set_title(
        f'MiniBooNE {model_name} — Bkg + Prediction vs Data\n'
...
def main():
    ...
    if global_best_params is not None:
        ...
        plot_bkg_pred_vs_data(
            ...
            savepath   = "miniboone_spectrum_bestfit.png",
        )
    ...
    #     savepath   = "miniboone_spectrum_custom.png",
    # )
