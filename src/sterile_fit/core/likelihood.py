"""core/likelihood.py: regrouped existing implementations; see docs/ARCHITECTURE.md."""
...
def solve_quadratic_form(residual: NDArray[np.float64], covariance: NDArray[np.float64]) -> float:
    ...
    try:
        lower_cholesky = cholesky(matrix, lower=True, check_finite=False)
    except np.linalg.LinAlgError as error:
        raise ValueError("total covariance is not positive definite; diagnose it explicitly before fitting") from error
    # Cholesky is triangular.  A general dense solve repeats unnecessary O(n^3)
    # work; the triangular solve is algebraically identical and O(n^2).
